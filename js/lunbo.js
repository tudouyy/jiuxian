//轮播图插件
class Carousel{
	constructor({el,timeout = 3000,isAuto,isDots,isXhx,isClick}){
		this.el = el;
		...
		if(this.isDots){
			...
			if(this.isXhx){
				//获取下划线的长度
				this.xhxs = this.el.getElementsByClassName('xhx')[0];
				this.xhxLen = this.dots.children.length;
			}	
	...
	xhxMove(){
		//左侧方向的判断
		if(this.xhxIndex<0){
			this.xhxIndex = this.xhxLen -1;
		}
		//右侧方向的判断
		if(this.xhxIndex>=this.xhxLen){
			this.xhxIndex = 0;
		}
		for(let i=0;i<this.xhxLen;i++){
			this.xhxs.children[i].className = '';
		}
		this.xhxs.children[this.xhxIndex].className='active2';
	}
}